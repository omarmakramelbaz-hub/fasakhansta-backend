...
use App\Http\Controllers\Api\V1\User\ProductWishlistController;
...
Route::group(['namespace' => 'Api', 'middleware' => ['CheckLang']], function () {
    ...
    Route::group(['middleware' => ['auth:api', 'custom.jwt']], function () {
            ...
            if (Auth::check()) {
                ...
                if ($channelName === 'private-user.' . Auth::id()) {
                    ...
                    return response()->json([
                        'auth' => $response['auth'],
                        'channel_data' => ['user_id' => (int) Auth::id()]
                    ]);
                }
        ...
        Route::group(['prefix' => 'user'], function () {
            ...
            Route::post('/product-wishlist/{resturantProduct}/toggle', [ProductWishlistController::class, 'toggle']);
