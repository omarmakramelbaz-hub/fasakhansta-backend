        ...
        if ($area) {
            $resturant = $resturant->select(\DB::raw('*, ( 6367 * acos( cos( radians('.$latitude.') ) * cos( radians( lat ) ) *
                        ...
                        ->where(function ($query) use ($area) {
                            $query->whereHas('resturant_areas', function ($q) use ($area) {
                                $q->where('area_id', $area->id);
                            })->orWhere('under_contract', 'yes');
                        });

            if ($area->cairo_id == 408) {
                $resturant = $resturant->orWhereHas('resturant_areas', function ($q) {
                    $q->whereHas('area',function($v){
                        $v->where('cairo_id', 408);
                    });
                });
            }
        }
