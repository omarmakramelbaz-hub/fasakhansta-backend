    ...
    public function getOrders(Request $request){
        ...
        if(! empty($request->status) ){
            if($request->status == 'completed'){
                $order = $order->whereIn('status',['cancelled','completed']);
        ...
        if(! empty($request->home) && $request->home == 'yes'){
            $today = Carbon::today();
            $order = $order->has('carts')->whereNotNull('status')->where('resturant_id',$resturant_id)->whereDate('created_at', $today)->orderBy('id','DESC')->paginate(5);
        }else{
            //all orders
            $order = $order->has('carts')->whereNotNull('status')->where('resturant_id',$resturant_id)->orderBy('id','DESC')->paginate(5);
        }
        ...
        if($order->status=='pending'||$order->status == 'accepted' || $order->status == 'another_delegate'){
            ...
            if($item){
                $item->update([
                 ...
                 ['tax'=>$tax,'vendor_tax'=>$vendor_tax,'user_tax'=>$user_tax,
                 ]);
