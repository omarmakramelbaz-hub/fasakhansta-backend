                ...
                <tbody>
                    <tr>
                        <td>
                            ...
                            <p>هذا هو تقريرك الشهري يتضمن تفاصيل الطلبات والمعلومات المهمة لمساعدتك في متابعة أعمالك بسهولة.</p>
                            <?php  
                                    $user = \App\Models\User::where('id',$vendor->id)->first();
                              $orders =\App\Models\Order::where('resturant_id', $user?->base_resturant->id)->whereNotNull('resturant_id')->where('type','!=','wallet')->where('status','completed')->whereMonth('created_at', now()->month)->get();
                            ?>
                            @if($orders->count()>0)
                            <a href="{{ route('download-daily-report-pdf',['orders' =>$orders,'vendor' => $vendor->id, 'download'=>'pdf','month'=>'month']) }}" class="main-btn">تحميل التقرير الشهرى</a>
                            @else
                            
                              <h3 class="note">لا يوجد طلبات لهذا الشهر</h3>
                            @endif
