                            ...
                            <div class="">
                                <form method="post" action="{{route('pending_vendors.transferVendor')}}" enctype="multipart/form-data">
                                    ...
                                    <div class="row gy-4">
                                        ...
                                        <legend>@lang('main.resturant_owner'):</legend>
                                        <input type="hidden" name="added_by[0]" value="{{auth('admin')->user()->id}}">
                                        <input type="hidden" name="email[0]" value="{{$pending->email}}">
                                        <input type="hidden" name="account_type[0]" value="resturant_owner">

                                          <div class="form-group col-sm-6">
                                            <label for="name"> @lang('main.name')</label><span class="text-danger">*</span>
                                            <input type="text" name="name[0]" required value=" {{ old('name,0', $pending->full_name) }} "
                                                class="form-control  @error('name') is-invalid @enderror" id="name">
                                        </div>
                                        <div class="form-group col-sm-6">
                                            <label for="mobile"> @lang('main.mobile')</label><span class="text-danger">*</span>
                                            <input type="text" name="mobile[0]" required value=" {{ old('mobile.0', $pending->mobile) }} " class="form-control @error('mobile') is-invalid @enderror"
                                                id="mobile">
                                        </div>
                                        <input type="hidden" name="roles_name[0][0]" value="resturant_owner"/>
                                        <div class="form-group col-sm-6">
                                            <label for="password"> @lang('main.Password')</label><span class="text-danger">*</span>
                                            <input type="password" name="password[0]" required value=""
                                    ...
                                    @for($i=1; $i<=$pending->branches_no; $i++)
                                    <div class=" col-md-6">
                                        <div class="card px-4 py-3">
                                           <fieldset class=" row gy-1" >
                                        ...
                                        <input type="hidden" name="account_type[{{$i}}]" value="vendor">
